...
const DynamicCategoryIcon = ({ iconData, className }: { iconData?: string, className?: string }) => {
  ...
  const IconComponent = (LucideIcons as any)[iconData] || LucideIcons.Folder;
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      ...
      try {
        ...
        // (Alternatively, if your backend returns `children` inside the category response, you can just use that)
          ...
          {products.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
              {products.map((product) => {
                // Safe extraction logic (no ProductCard required)
