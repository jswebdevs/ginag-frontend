...
export default function RefundsPage() {
  ...
  const fetchRefundOrders = async (tab: string) => {
    ...
    try {
      if (tab === "ALL REFUNDS") {
        // Fetch all 3 negative statuses simultaneously and merge them
        ...
        // Fetch specific tab status
        const res = await api.get(`/orders?limit=100&status=${tab}`);
      ...
      <div className="bg-card border border-border rounded-3xl shadow-theme-sm overflow-hidden min-h-[50vh]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-muted-foreground font-bold">No refund or cancellation records found.</p>
          </div>
        ) : (
