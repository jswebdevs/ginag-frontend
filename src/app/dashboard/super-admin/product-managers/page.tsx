...
import { Search, Loader2, UserCheck, Plus } from "lucide-react";
...
export default function ProductManagersPage() {
    ...
    const fetchUsers = async () => {
        try {
            const res = await api.get('users/role/product_manager');
    ...
    const handleDelete = async (id: string) => {
        const result = await Swal.fire({
            title: 'Remove Staff?',
            text: "This product manager will lose all access immediately.",
            icon: 'warning',
        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto animate-in fade-in duration-500 pb-24">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Manage staff responsible for catalog and inventory.</p>
                </div>
            ...
            <div className="bg-card border border-border rounded-3xl shadow-theme-sm overflow-hidden min-h-[50vh]">
                {loading ? (
                    ...
                    <UserTable users={filteredUsers} onDelete={handleDelete} basePath="/dashboard/super-admin/product-managers" />
                )}
            </div>
        </div>
    );
