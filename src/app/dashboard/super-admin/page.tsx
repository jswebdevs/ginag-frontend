...
export default function SuperadminDashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        // Fetch from your existing routes concurrently!
        const [overviewRes, chartRes, ordersRes] = await Promise.all([
          api.get('/analytics/overview'),
          api.get(`/analytics/chart-data?days=${days}`),
          api.get('/orders/all?limit=5') // Your existing Admin order route
        ]);
    ...
    fetchDashboardData();
  }, [days]); // Removed 'data' from dependency array to prevent infinite loops
  ...
  if (error) return <div className="p-8 text-red-500 bg-background min-h-screen font-bold">{error}</div>;

  return (
    <div className="space-y-6 transition-colors duration-300 animate-in fade-in slide-in-from-bottom-4">
      <WelcomeCard3D />

      {/* Passes the overview data directly */}
