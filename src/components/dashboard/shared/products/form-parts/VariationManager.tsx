...
export default function VariationManager({ product, update }: any) {
  ...
  // ==========================================
  // 1. ATTRIBUTE BUILDER LOGIC
  // ==========================================
  const addAttribute = () => {
    update({ attributes: [...(product.attributes || []), { name: "", values: [] }] });
  };
  ...
  const generateMatrix = async () => {
    ...
    if (validAttrs.length === 0) {
      return Swal.fire("Missing Data", "Please add at least one attribute with values first.", "warning");
    }

    const result = await Swal.fire({
      title: 'Generate Variations?',
      text: "This will overwrite your current variations list.",
      icon: 'warning',
    ...
    <div className="bg-card border border-border rounded-3xl p-6 shadow-theme-sm space-y-6">
      ...
      <div className="space-y-4">
        ...
        {(product.attributes || []).length === 0 && (
          <div className="text-center p-4 border border-dashed border-border rounded-xl text-xs text-muted-foreground italic">
            Add attributes like "Size" or "Color" to generate variations.
          </div>
        )}
        ...
        <div className="space-y-3">
          {(product.attributes || []).map((attr: any, i: number) => (
            <div key={i} className="bg-muted/10 border border-border rounded-xl p-3 space-y-3 relative group">
              <button
                type="button" onClick={() => removeAttribute(i)}
              ...
              <div>
                ...
                <input
                  type="text" value={attr.name} onChange={(e) => updateAttrName(i, e.target.value)}
                  placeholder="e.g., Color, Size"
                  className="w-full bg-background border border-border rounded-lg px-3 py-1.5 text-xs outline-none focus:border-primary font-bold"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {attr.values.map((val: string, vIdx: number) => (
                    <span key={vIdx} className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1">
                      {val}
                      <button type="button" onClick={() => removeValue(i, vIdx)} className="hover:text-destructive"><X size={10} /></button>
                    </span>
                  ))}
                </div>
                <div className="flex gap-2">
                  <input
                    type="text" value={inputValue[i] || ""}
                  ...
                  />
                  <button type="button" onClick={() => addValue(i, inputValue[i] || "")} className="bg-foreground text-background px-3 rounded-lg text-xs font-bold">
                    Add
                  </button>
                </div>
              </div>
        ...
        <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-1">
          ...
          {(product.variations || []).map((v: any, i: number) => (
            <div key={i} className={`bg-background border rounded-xl p-4 relative transition-all ${v.isDefault ? 'border-primary ring-1 ring-primary/20' : 'border-border'}`}>
              ...
              <button type="button" onClick={() => deleteVar(i)} className="absolute top-3 right-3 text-muted-foreground hover:text-destructive transition-colors">
                <Trash2 size={14} />
              </button>

              <div className="flex items-center gap-2 mb-3 pr-6">
                <div className="w-5 h-5 bg-primary/10 text-primary rounded-md flex items-center justify-center text-[10px] font-black shrink-0">
                  {i + 1}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="space-y-1">
                  ...
                  <input type="text" value={v.sku} onChange={(e) => updateVar(i, 'sku', e.target.value)} className="w-full bg-muted/30 border border-border rounded-md px-2 py-1 text-xs outline-none focus:border-primary" />
                </div>
                <div className="space-y-1">
              ...
              <label className="flex items-center gap-2 cursor-pointer group w-max">
                <input
                  type="radio" name="default_var_radio"
                  checked={v.isDefault}
