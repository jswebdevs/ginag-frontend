...
export default function FAQSection({ data: initialData }: { data?: any }) {
  ...
  useEffect(() => {
    if (!initialData) {
      ...
      api.get("/settings/homepage")
        .then(res => setData(res.data?.data?.faq))
        .catch(() => {})
  ...
  if (loading || !data) {
    return (
      <section className="py-24 bg-background relative overflow-hidden" aria-label="FAQ loading">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16 space-y-4">
            <div className="h-4 w-16 bg-muted/40 rounded-full mx-auto animate-pulse" aria-hidden="true" />
            <div className="h-12 w-96 bg-muted/40 rounded-full mx-auto animate-pulse" aria-hidden="true" />
          </div>
          <div className="max-w-3xl mx-auto space-y-4" aria-hidden="true">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-20 bg-muted/10 border border-border/50 rounded-2xl animate-pulse" />
            ))}
          </div>
        </div>
      </section>
  ...
  const faqs: { question: string; answer: string }[] = data.faqs?.length > 0 ? data.faqs : [];

  return (
    <section
      className="py-24 bg-background text-foreground relative overflow-hidden transition-colors duration-500"
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full flex items-center justify-between p-6 text-left gap-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-2xl"
                >
